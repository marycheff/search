      ...
      function performMovieSearch(movieName) {
        ...
        fetch(apiUrl, options)
          ...
          .then((data) => {
            ...
            let russia = movie.countries.some((country) => country.name === "Россия");

            russia = true;
            if (russia) {
              resultHTML += `<div id="single" class="kinobox_player"></div>`;
            } else {
              resultHTML += `
                <div class="wrapper">
          ...
          .catch((error) => {
            console.error(error);
            searchResultsElement.innerHTML = "<p>Ошибка при запросе к API Kinopoisk</p>";
          });
      }
